...
What it does:
1) Reads a local cbb26.csv backbone file.
2) Downloads several public Bart Torvik CSV endpoints/fallbacks.
3) Cleans/normalizes team names.
4) Merges everything into one 2026-only reduced feature file.
5) Saves raw pulls too, so you can inspect what actually came back.
...
MANUAL_ALIASES = {
    # common cross-source team-name differences
    "uconn": "connecticut",
    "unc wilmington": "uncw",
...
RAW_SOURCES = {
    # Bart has publicly documented that teamslicejson can return regular-season data as CSV,
    # and that adding csv=1 works on team-tables/team pages.
    "ratings": [
...
def read_table_from_text(text: str) -> pd.DataFrame:
    ...
    # Most Bart CSV endpoints are comma-separated. sep=None makes this more tolerant.
...
def fetch_first_csv(session, url_templates: List[str], year: int) -> Tuple[pd.DataFrame, str]:
    ...
    for template in url_templates:
        ...
        try:
            ...
            time.sleep(1)
    joined = "\n".join(errors)
    raise RuntimeError(f"All URL attempts failed:\n{joined}")
...
def clean_source_df(df: pd.DataFrame, source_name: str) -> pd.DataFrame:
    ...
    # Remove obviously empty columns.
    empty_cols = [c for c in df.columns if df[c].isna().all()]
    if empty_cols:
        df = df.drop(columns=empty_cols)

    # Prefix non-key columns so you keep provenance and avoid collisions.
    protected = {"team_name_raw", "team_name_norm"}
    rename_map = {c: f"{source_name}_{c}" for c in df.columns if c not in protected}
    df = df.rename(columns=rename_map)

    # One row per normalized team if possible.
...
def merge_source(base: pd.DataFrame, source: pd.DataFrame, source_name: str) -> pd.DataFrame:
    ...
    # cleanup
    drop_cols = [f"_merge_key_{source_name}"]
    for c in ["team_name_norm_" + source_name + "_dup", f"team_name_norm_{source_name}_dup"]:
        if c in merged.columns:
            drop_cols.append(c)
    if "team_name_norm_y" in merged.columns:
        drop_cols.append("team_name_norm_y")
    if "team_name_norm_x" in merged.columns:
        merged = merged.rename(columns={"team_name_norm_x": "team_name_norm"})
    if "team_name_raw" in merged.columns and f"{source_name}_team_name_raw" not in merged.columns:
        # if an unprefixed raw column slipped through, rename it
        merged = merged.rename(columns={"team_name_raw": f"{source_name}_team_name_raw"})
...
def choose_columns(df: pd.DataFrame) -> pd.DataFrame:
    # Keep everything useful but surface a cleaner reduced-model block up front.
    preferred_front = [
        "season",
        "conference",
        "team_name",
...
def main() -> int:
    ...
    unmatched = merged[merged.filter(regex=r"^(ratings_|teamstats_|team_tables_|teamsheets_|sos_)").isna().all(axis=1)][
    ...
    print("[5/5] Done.")
    print("\nIf a Bart source failed, open the build report JSON and inspect which URL was blocked.")
    print("You can still model with the columns that did merge successfully.")
    return 0
